...
namespace E_Commerce.Infrastructure.DataSeedig
{
    internal class CatalogDataSeeder(StoreDbContext dbContext,ILogger<CatalogDataSeeder> logger) : IDataSeeder
    {
        public async Task SeedDataAsyc(CancellationToken ct = default)
        {
            try
            {
                var pedingMigratios = await dbContext.Database.GetPendingMigrationsAsync(ct); //Collection of names Migrations
                if (pedingMigratios.Any())
                   await dbContext.Database.MigrateAsync(ct);

                //Seeding
                //path
                //G:\Projects\MVC\E_Commerce\E_Commerce.API\bin\Debug\net8.0\DataSeed
        ...
        private async Task SeedIfEmptyAsync<T,Tkey>(string rootPath ,string fielname ,CancellationToken ct = default) where T : BaseEntity<Tkey>
        {
            ...
            var filePath = Path.Combine(rootPath, fielname);

            if (!File.Exists(filePath))
            {
                logger.LogInformation($"File {fielname} Is Not Exists");
                return;
            }
